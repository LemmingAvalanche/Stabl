...
checkTopDefinitions program = validWordCalls program set where set = builtInWords `Set.union` (Set.fromList $ map fst program)
...
validWordCall (name,quot) set = if name `Set.member` set
                         then Nothing 
                         else Just $ "Error: word " ++ name ++ " is not a defined word. Was called in the definition: " ++ "\n\t" ++ (show quot)